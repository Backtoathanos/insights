...
function parseRss($config)
{
    ...
    // Get the RSS content
    $rssContent = \Acelle\Helpers\url_get_contents_ssl_safe($config['url']);
    if ($rssContent === false) {
        throw new Exception("Failed to fetch RSS content from URL: " . $config['url']);
    }
    $rssContent = preg_replace('/<!DOCTYPE[^>]+>/', '', $rssContent);

    // Load XML and check for errors
    libxml_use_internal_errors(true); // Enable custom error handling
    $rssFeed = simplexml_load_string($rssContent, 'SimpleXMLElement', LIBXML_NOCDATA);
    ...
    if ($rssFeed === false) {
        ...
        libxml_clear_errors();
        throw new Exception("Failed to parse RSS XML: " . implode(", ", $errorMessages));
    }

    // Convert to array (with validation)
    $rssArray = xml_to_array($rssFeed);

    // Take 10 records only (with fallback if structure is different)
    $records = $rssArray['rss']['channel']['item'] ?? [];
    $records = array_slice($records, 0, $config['size']);

    // feed data (with null checks)
    $feedData = [];
    $feedData['feed_title'] = (string) ($rssFeed->channel->title ?? '');
    $feedData['feed_description'] = $rssFeed->channel->description ? $rssFeed->channel->description->__toString() : '';
    $feedData['feed_link'] = $rssFeed->channel->link ? $rssFeed->channel->link->__toString() : '';
    $feedData['feed_pubdate'] = $rssFeed->channel->pubDate ? $rssFeed->channel->pubDate->__toString() : '';
    $feedData['feed_build_date'] = $rssFeed->channel->lastBuildDate ? $rssFeed->channel->lastBuildDate->__toString() : '';
    ...
    // records (with null checks)
    $rss['items'] = [];
    $count = 0;
    if (!empty($rssFeed->channel->item)) {
        foreach ($rssFeed->channel->item as $item) {
            // item data (with fallbacks)
            $itemData = [
                'item_title' => (string) ($item->title ?? ''),
                'item_pubdate' => (string) ($item->pubDate ?? ''),
                'item_description' => (string) ($item->description ?? ''),
                'item_url' => (string) ($item->link ?? ''),
                'item_enclosure_url' => (string) ($item->enclosure['url'] ?? ''),
                'item_enclosure_type' => (string) ($item->enclosure['type'] ?? ''),
            ];
    
            // item parse template
            $parsedItem = [
                'ItemTitle' => parseRssTemplate($config['templates']['ItemTitle']['template'], $itemData),
                'ItemDescription' => parseRssTemplate($config['templates']['ItemDescription']['template'], $itemData),
                'ItemMeta' => parseRssTemplate($config['templates']['ItemMeta']['template'], $itemData),
                'ItemEnclosure' => parseRssTemplate($config['templates']['ItemEnclosure']['template'], $itemData),
                'ItemStats' => parseRssTemplate($config['templates']['ItemStats']['template'], $itemData),
            ];
    
            $rss['items'][] = $parsedItem;
