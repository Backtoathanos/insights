...
const apiUrl = "https://adminapi.blackridgeresearch.com/blog/live/projectprofiles?page=1&limit=1000000";
...
async function GET(context) {
  ...
  return rss({
    ...
    title: "Blackridge Research and Consulting | Project Profiles",
    ...
    site: context.site + "/project-profiles",
    trailingSlash: false,
    ...
    <atom:link href="${context.site}rss_project_profiles.xml" rel="self" type="application/rss+xml" />`,
    // items (each post)
    items: posts.posts.map((post) => ({
      // link: getRelativeLocaleUrl(rssLocale, `/project-profiles/${post.page_title}/`),
      // title: post.title,
      ...
      customData: `
                <link>${url + "project-profiles/" + post.page_title}</link>
                ...
                <guid isPermaLink="true">${url + "project-profiles/" + post.page_title}</guid>
                <category>Project Profiles</category>
            `
