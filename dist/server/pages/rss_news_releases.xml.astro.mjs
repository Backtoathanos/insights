...
const apiUrl = "https://adminapi.blackridgeresearch.com/blog/live/newsrelease?page=1&limit=1000000";
...
async function GET(context) {
  ...
  return rss({
    ...
    title: "Get the Latest Industry and Project-Related News | Blackridge Research and Consulting",
    site: context.site + "/news-releases",
    trailingSlash: false,
    ...
    <atom:link href="${context.site}rss_news_releases.xml" rel="self" type="application/rss+xml" />`,
    // items (each post)
    items: posts.posts.map((post) => ({
      // link: getRelativeLocaleUrl(rssLocale, `/news-releases/${post.page_title}/`),
      // title: post.title,
      // description: post.meta_desc,
      // pubDate: post.last_updated,
      // guid: btoa(post.nid),
      customData: `
                <link>${url + "news-releases/" + post.page_title}</link>
                ...
                <guid isPermaLink="true">${url + "news-releases/" + post.page_title}</guid>
                <category>${post.styp2}</category>
            `
